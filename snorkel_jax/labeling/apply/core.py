...
from snorkel_jax.labeling.lf import LabelingFunction
from snorkel_jax.types import DataPoint, DataPoints
from snorkel_jax.utils.data_operators import check_unique_names
...
class BaseLFApplier:
    ...
    def _numpy_from_row_data(self, labels: List[RowData]) -> jnp.array:
        L = jnp.zeros((len(labels), len(self._lfs)), dtype=int) - 1
        # NB: this check will short-circuit, so ok for large L
        if any(map(len, labels)):
            row, col, data = zip(*chain.from_iterable(labels))
            L=L.at[row, col].set(data)
...
class LFApplier(BaseLFApplier):
    ...
    def apply(
        self,
        data_points: Union[DataPoints, jnp.array],
        progress_bar: bool = True,
        fault_tolerant: bool = False,
        return_meta: bool = False,
    ) -> Union[jnp.array, Tuple[jnp.array, ApplierMetadata]]:

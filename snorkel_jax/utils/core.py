...
def to_int_label_array(X: jnp.array, flatten_vector: bool = True) -> jnp.array:
    ...
    """
    if jnp.any(jnp.not_equal(jnp.mod(X, 1), 0)):
...
def filter_labels(
    label_dict: Dict[str, jnp.array], filter_dict: Dict[str, List[int]]
) -> Dict[str, jnp.array]:
    ...
    mask = (jnp.multiply(*masks) if len(masks) > 1 else masks[0]).squeeze()
...
def _get_mask(label_array: jnp.array, filter_values: List[int]) -> jnp.array:
    ...
    """
    mask = jnp.ones_like(label_array).astype(bool)
